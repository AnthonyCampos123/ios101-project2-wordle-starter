  ...
  private func applyThemeSettings(with settings: [String: Any]) {
    // START YOUR CODE HERE
      if let rawTheme = settings[kWordThemeKey] as? String {
              // Convert the string to the appropriate WordTheme enum case using WordTheme(rawValue:)
              if let theme = WordTheme(rawValue: rawTheme) {
                  // Generate the goal word based on the theme and assign it to goalWord
  ...
  private func applyIsAlienWordleSettings(with settings: [String: Any]) {
    // START YOUR CODE HERE
      if let isAlienWordleSetting = settings[kIsAlienWordleKey] as? Bool {
              // Assign the value of isAlienWordleSetting to the isAlienWordle property
